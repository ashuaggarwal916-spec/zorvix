// Zorvix - GitHub Pages + Supabase (Multi-user, permanent)
const STORAGE_KEY = 'zorvix_sb_config';
let supabase = null;
let leads = [];
...
function updateStats() {
  ...
  for (const l of leads) {
    ...
    if (l.status === 'interested') s.interested++;
  }
...
function renderLeads() {
  ...
  if (!leads.length) {
    c.innerHTML = '<div class="empty"><div class="empty-icon">📭</div><p>No leads yet</p><p style="font-size:0.8rem;margin-top:4px;">Tap + to upload CSV</p></div>';
    return;
  }
  c.innerHTML = leads.slice(0, 50).map(l => `
    <div class="lead" onclick="openDetail('${l.id}')">
      <div class="lead-top">
        <div>
          <div class="lead-name">${l.name}</div>
          <div class="lead-meta">${l.business_name || ''} ${l.city ? '• ' + l.city : ''}</div>
        </div>
      ...
      <div class="lead-actions" onclick="event.stopPropagation()">
        ...
        <a href="https://wa.me/${l.phone.replace('+', '')}?text=Hi ${l.name}! Main Zorvix se call kar raha hoon." target="_blank" class="btn btn-wa btn-sm">💬</a>
...
async function uploadCSV(event) {
  ...
  if (!supabase) return toast('Please connect Supabase first', true);
  ...
  if (error) return toast('Upload failed', true);
  
  toast('✅ ' + rows.length + ' leads added');
...
function openDetail(id) {
  ...
  if (!l) return;
  const msg = prompt(`Message for ${l.name}?`, `Hi ${l.name}! Aapki website ban gayi hai. Dekhiye!`);
  if (msg) {
    window.open(`https://wa.me/${l.phone.replace('+', '')}?text=${encodeURIComponent(msg)}`, '_blank');
    updateStatus(id, 'reached');
  }
}

async function autoCall() {
  if (!supabase) return toast('Connect Supabase first', true);
  const pending = leads.filter(l => l.status === 'pending');
  if (!pending.length) return toast('No pending leads', true);
  if (!confirm('Call first ' + Math.min(5, pending.length) + ' leads?')) return;
  
  for (const l of pending.slice(0, 5)) {
    updateStatus(l.id, 'calling');
  }
  toast('📞 Calls queued');
}
...
function saveSupabase() {
  ...
  if (!url || !key) return toast('Enter URL and Key', true);
