// Zorvix - Auto-configured Supabase
...
async function loadLeads() {
  ...
  if (error) {
    if (error.message.includes('relation')) {
      document.getElementById('leadsList').innerHTML = `<div class="empty"><div class="empty-icon">⚠️</div><p>Table missing</p><p style="font-size:0.75rem;margin-top:6px;">Run this SQL in Supabase:</p><textarea readonly style="width:100%;height:60px;font-size:0.65rem;background:var(--bg);border:1px solid var(--border);border-radius:6px;color:var(--text);padding:6px;margin-top:8px;">create table leads (id uuid default gen_random_uuid() primary key, name text, phone text, business_name text, city text, service text, status text default 'pending', attempts int default 0, notes text default '', created_at timestamptz default now());</textarea></div>`;
      return;
    }
  }
  leads = data || [];
